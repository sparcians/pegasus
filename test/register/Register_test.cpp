...
const uint64_t ALTERNATING_DEFAULT[] = {0xaaaaaaaaaaaaaaaa, 0xaaaaaaaaaaaaaaaa, 0xaaaaaaaaaaaaaaaa,
...
Register::Definition reg_defs[] = {
    ...
    {11,
     ...
     "partially masked",
     4,
     {{"b03_00", "A", 0, 3, true}, {"b09_05", "B", 5, 9, true}, {"b15_12", "C", 12, 15, false}},
     {},
     nullptr,
     Register::INVALID_ID,
     0,
     (const uint8_t*)ALTERNATING_DEFAULT,
     0,
     0},
    {12,
     "wm_02",
    ...
    {13,
     ...
     "fully writable",
     16,
     {{"b127_64", "A", 64, 127, false}, {"b63_00", "B", 0, 63, false}},
     {},
     nullptr,
     Register::INVALID_ID,
     0,
    ...
    {14,
     ...
     "mask spans u64s",
     16,
     {{"b65_13", "A", 13, 65, true}, {"b67_64", "B", 64, 67, false}},
     {},
     nullptr,
     Register::INVALID_ID,
     0,
...
RegisterProxy::Definition proxy_defs[] = {{302, "sprxx", 4, "D", 2, "example SPR PROXY"},
...
class DummyDevice : public sparta::TreeNode
{
  public:
    DummyDevice(sparta::TreeNode* parent) :
        sparta::TreeNode(parent, "dummy", "", sparta::TreeNode::GROUP_IDX_NONE,
                         "dummy node for register test")
    {
    }
};

template <typename RegReadSizeT> class RegPostWriteObserver
{
  public:
    uint32_t writes_1;
    uint32_t writes_2;
    ...
    RegPostWriteObserver() : writes_1(0), writes_2(0), pre(0), post(0) {}
    ...
    void registerForCb1(RegisterBase* r)
    {
        ...
        // r->getPostWriteNotificationSource().registerForThis<RegPostWriteObserver,
        // &RegPostWriteObserver<RegReadSizeT>::callback1>(this);
        // r->getPostWriteNotificationSource().registerForThis<RegPostWriteObserver,
        // &RegPostWriteObserver::callbackTemplate<int, int>>(this);
    }

    void deregisterForCb1(RegisterBase* r)
    {
        r->getPostWriteNotificationSource().DEREGISTER_FOR_THIS(callback1);
        // r->getPostWriteNotificationSource().deregisterForThis<RegPostWriteObserver,
        // &RegPostWriteObserver::callback1>(this);
    }
    ...
    void callback1(const sparta::TreeNode & origin, const sparta::TreeNode & obs_pt,
                   const Register::PostWriteAccess & data)
    {
        (void)origin;
        (void)obs_pt;
    ...
    void callbackTemplate(const sparta::TreeNode &, const sparta::TreeNode &,
                          const Register::PostWriteAccess &)
    {
    }
};

template <typename RegReadSizeT> class FastRegPostWriteObserver
{
  public:
    uint32_t writes;

    FastRegPostWriteObserver() : writes(0) {}
    ...
    void callback(const sparta::TreeNode & origin, const sparta::TreeNode & obs_pt,
                  const Register::PostWriteAccess & data)
    {
        (void)origin;
        (void)obs_pt;
...
template <typename RegReadSizeT> class RegReadObserver
{
  public:
    uint32_t reads;
    RegReadSizeT expected;

    RegReadObserver() : reads(0), expected(0) {}

    void expect(RegReadSizeT _expected) { expected = _expected; }

    void registerFor(RegisterBase* r)
    {
        r->getReadNotificationSource().REGISTER_FOR_THIS(callback);
        // r->getReadNotificationSource().registerForThis<RegReadObserver,
        // &RegReadObserver::callback>(this);
    ...
    void deregisterFor(RegisterBase* r)
    {
        ...
        // r->getReadNotificationSource().deregisterForThis<RegReadObserver,
        // &RegReadObserver::callback>(this);
    }

    void callback(const sparta::TreeNode & origin, const sparta::TreeNode & obs_pt,
                  const Register::ReadAccess & data)
    {
        (void)origin;
        (void)obs_pt;
...
template <typename T> class CallbackDummy
{
  public:
    ...
    void callback2(const T & data) { (void)data; }
};
...
void testBadRegs()
{
    ...
    for (uint32_t & sz : sizes)
    {
        ...
        if (sz == 0)
        {
            fname = "zero_byte_reg.json";
        }
        else if (sz == 3)
        {
            fname = "non_power_of_2_count_reg.json";
        }
        else if (sz == 9)
        {
            fname = "odd_byte_count_reg.json";
        }
...
void testRegFileNoThrow(const std::string & filename)
{
    RootTreeNode root;
...
int main()
{
    ...
    EXPECT_TRUE(
        rset->isAttached()); // Ensure that node constructed with parent arg is properly attached
    ...
    EXPECT_THROW(rset->getRegister(
        "there_is_no_register_by_this_name_here_or_anywhere")); // No reg by this name here
    ...
    std::cout << "wm_01 write-mask: " << rset->getRegister("wm_01")->getWriteMaskAsByteString()
              << std::endl;
    std::cout << "wm_01 write-mask: " << rset->getRegister("wm_01")->getWriteMaskAsBitString()
              << std::endl;
    EXPECT_EQUAL(rset->getRegister("wm_01")->getWriteMask<uint16_t>(1), 0xffff);
    EXPECT_EQUAL(rset->getRegister("wm_01")->getWriteMask<uint16_t>(0),
                 0xfc10); // masked b03_00 b09_05
    ...
    std::cout << "wm_02 write-mask: " << rset->getRegister("wm_02")->getWriteMaskAsByteString()
              << std::endl;
    std::cout << "wm_02 write-mask: " << rset->getRegister("wm_02")->getWriteMaskAsBitString()
              << std::endl;
    ...
    std::cout << "wm_03 write-mask: " << rset->getRegister("wm_03")->getWriteMaskAsByteString()
              << std::endl;
    std::cout << "wm_03 write-mask: " << rset->getRegister("wm_03")->getWriteMaskAsBitString()
              << std::endl;
    ...
    std::cout << "wm_04 write-mask: " << rset->getRegister("wm_04")->getWriteMaskAsByteString()
              << std::endl;
    EXPECT_EQUAL(rset->getRegister("wm_04")->getWriteMaskAsByteString(),
                 "ff ff ff ff ff ff ff fc 00 00 00 00 00 00 1f ff");
    std::cout << "wm_04 write-mask: " << rset->getRegister("wm_04")->getWriteMaskAsBitString()
              << std::endl;
    EXPECT_EQUAL(rset->getRegister("wm_04")->getWriteMaskAsBitString(),
                 "11111111 11111111 11111111 11111111 11111111 11111111 11111111 11111100 00000000 "
                 "00000000 00000000 00000000 00000000 00000000 00011111 11111111");
    ...
    std::cout << "Registers: " << large << " " << med << " " << notareg << std::endl;
    ...
    // Create good fields
    Register::Field *starting = 0, *almost_ending = 0, *ending = 0, *spanning = 0, *notafield = 0,
                    *span_large = 0, *span_largest = 0;
    // EXPECT_NOTHROW(full = large->addField(Register::Field::Definition{"full_field", "description
    // for this field", 0, 127}));
    ...
    Register::Field::Definition f_almost_ending{"almost_ending_field", "description for this field",
                                                124, 125};
    ...
    EXPECT_NOTHROW(
        spanning = large->addField(
            f_spanning)); // Spans 64b boundary. This is an edge case for field reading/writing
    Register::Field::Definition f_span_large{"spanning_field_large", "description for this field",
                                             60, 75};
    EXPECT_NOTHROW(span_large =
                       large->addField(f_span_large)); // Spans 64b with more bits boundary. This is
                                                       // an edge case for field reading/writing
    Register::Field::Definition f_span_largest{"spanning_field_largest",
                                               "description for this field", 33, 90};
    EXPECT_NOTHROW(
        span_largest = large->addField(
            f_span_largest)); // Spans 64b with more bits boundary and does not start or end on a
                              // nibble. This is an edge case for field reading/writing

    // Field printing by pointer
    std::cout << "Fields: " << starting << " " << ending << " " << spanning << " " << notafield
              << std::endl;
    ...
    Register::Field::Definition f_illegal{"illegal_generated_field_1", "description for this field",
                                          0, 128};
    ...
    EXPECT_THROW(large->addAlias(
        "alias_name_that_shouldnt_exist")); // Already has a parent node; Cannot add aliases

    EXPECT_EQUAL(root.getPhase(), sparta::TreeNode::TreePhase::TREE_BUILDING);
    EXPECT_FALSE(
        root.isBuilt()); // Make sure we didn't accidentally move away from the TREE_BUILDING phase
    ...
    testRegFileNoThrow("reg_vec256.json");
    testRegFileNoThrow("reg_vec512.json");
    testRegFileNoThrow("reg_vec1024.json");
    testRegFileNoThrow("reg_vec2048.json");
    ...
    EXPECT_EQUAL(b->getNotificationID(),
                 sparta::StringManager::getStringManager().internString("post_write"));
    ...
    EXPECT_EQUAL(b->getNotificationID(),
                 sparta::StringManager::getStringManager().internString("post_read"));
    ...
    EXPECT_EQUAL(med->getPossibleSubtreeNotifications(infos),
                 2); // post_write and post_read notifications per register
    EXPECT_EQUAL(infos.size(), 2);
    EXPECT_EQUAL(rset->getPossibleSubtreeNotifications(infos), 2 * rset->getNumRegisters());
    EXPECT_EQUAL(infos.size(),
                 2 + 2 * rset->getNumRegisters()); // 2 notis per register. 2 already in infos.
    ...
    EXPECT_EQUAL((rset->locateNotificationSources(srcs, "not_a_notification_name")), 0);
    EXPECT_EQUAL(srcs.size(), 0);
    ...
    EXPECT_THROW((
        rset->registerForNotification<uint32_t, decltype(cbc), &CallbackDummy<uint32_t>::callback1>(
            &cbc, "a_notification", true)));

    CallbackDummy<sparta::Register::PostWriteAccess> write_cb;
    EXPECT_NOTHROW((
        rset->registerForNotification<sparta::Register::PostWriteAccess, decltype(write_cb),
                                      &CallbackDummy<sparta::Register::PostWriteAccess>::callback1>(
            &write_cb, "")));
    EXPECT_NOTHROW((rset->deregisterForNotification<
                    sparta::Register::PostWriteAccess, decltype(write_cb),
                    &CallbackDummy<sparta::Register::PostWriteAccess>::callback1>(&write_cb, "")));
    EXPECT_THROW((
        rset->registerForNotification<sparta::Register::PostWriteAccess, decltype(write_cb),
                                      &CallbackDummy<sparta::Register::PostWriteAccess>::callback1>(
            &write_cb, "not_a_notification")));

    EXPECT_NOTHROW((
        rset->registerForNotification<sparta::Register::PostWriteAccess, decltype(write_cb),
                                      &CallbackDummy<sparta::Register::PostWriteAccess>::callback2>(
            &write_cb, "")));
    EXPECT_NOTHROW((rset->deregisterForNotification<
                    sparta::Register::PostWriteAccess, decltype(write_cb),
                    &CallbackDummy<sparta::Register::PostWriteAccess>::callback2>(&write_cb, "")));
    EXPECT_THROW((
        rset->registerForNotification<sparta::Register::PostWriteAccess, decltype(write_cb),
                                      &CallbackDummy<sparta::Register::PostWriteAccess>::callback2>(
            &write_cb, "not_a_notification")));
    ...
    std::cout << " have: " << std::hex << med->read<uint64_t>() << " expect: " << 0xccddeeeeccddffff
              << std::endl;
    ...
    std::cout << " now: " << large << std::endl;
    std::cout << " have: " << std::hex << large->read<uint64_t>()
              << " expect: " << 0xccddeeeeccddffff << std::endl;
    ...
    std::cout << " have: " << std::hex << huge->read<uint64_t>()
              << " expect: " << 0xccddeeeeccddffff << std::endl;
    ...
    uint64_t large_val[2] = {large->read<uint64_t>(0),
                             large->read<uint64_t>(1)}; // C++11 is convenient
    EXPECT_EQUAL((rset->getRegister("large_x0")->read<uint32_t>()),
                 (uint32_t)((large_val[0] & 0xffffffff)));
    EXPECT_EQUAL((rset->getRegister("large_x4")->read<uint32_t>()),
                 (uint32_t)((large_val[0] & 0xffffffff00000000) >> 32));
    EXPECT_EQUAL((rset->getRegister("large_x8")->read<uint32_t>()),
                 (uint32_t)((large_val[1] & 0xffffffff)));
    EXPECT_EQUAL((rset->getRegister("large_xC")->read<uint32_t>()),
                 (uint32_t)((large_val[1] & 0xffffffff00000000) >> 32));
    EXPECT_EQUAL((rset->getRegister("large_x3")->read<uint32_t>()),
                 (uint32_t)((large_val[0] & 0xffffffff000000) >> 24));
    ...
    std::cout << " wm_04 wm  " << rset->getRegister("wm_04")->getWriteMaskAsByteString()
              << std::endl;
    ...
    std::cout << "\nIllegal access tests: max_index: 0x" << std::hex << max_index << std::dec
              << std::endl;
    ...
    EXPECT_EQUAL(sprxxa->dmiRead<uint32_t>(),
                 0xc5acce55); // read directly from register's backing store
