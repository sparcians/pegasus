...
void testFieldRegisterWrite()
{
    ...
    std::unique_ptr<atlas::RegisterSet> regs = atlas::RegisterSet::create(&good_dummy, "fp_reg.json");
...
void testGoodRegs()
{
    RootTreeNode root;
    DummyDevice good_dummy(&root);
    std::unique_ptr<atlas::RegisterSet> good_regs = atlas::RegisterSet::create(&good_dummy, "int_reg.json");
    root.enterTeardown();
}
...
int main()
{
    ...
    std::unique_ptr<RegisterSet> rset(
        RegisterSet::create(&dummy, reg_defs));
    ...
    // Test the script-generated reg_*.json files (this is basically testing the script, not the registers)
