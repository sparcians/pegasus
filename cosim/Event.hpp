...
namespace atlas::cosim
{
    ...
    public:
        ...
        struct RegReadAccess {
            ...
            RegReadAccess(RegId id, const std::vector<uint8_t>& val) :
                reg_id(id),
                value(val)
            {}
        };
        ...
        struct RegWriteAccess : public RegReadAccess {
            ...
            RegWriteAccess(RegId id, const std::vector<uint8_t>& val, const std::vector<uint8_t>& prev_val) :
        ...
        //! @{
            ...
            Event(uint64_t euid, Type etype = Type::INVALID) :
                event_uid_(euid),
                type_(etype)
            {}

            Event(const Event& other) :
                event_uid_(other.event_uid_),
                type_(other.type_)
            {
                // Copy all members
    ...
    private:
        ...
        // Region of interest
        bool is_in_region_of_interest_ {true};        //!< True if this Event occurred in the region of interest
        bool is_entering_region_of_interest_ {false}; //!< True if the next Event will be in the region of interest
        bool is_exiting_region_of_interest_ {false};  //!< True if the next Event will not be in the region of interest
        ...
        OpcodeSize opcode_size_ = std::numeric_limits<OpcodeSize>::max(); //!< Opcode size for instruction Events

        // Atlas instruction type
        InstType inst_type_ = InstType::INVALID; //!< Instruction type for instruction Events

        // PC
        bool is_change_of_flow_ {false};                            //!< True if this Event changed the PC (e.g. taken branches, system calls, traps)
        Addr curr_pc_ = std::numeric_limits<Addr>::max();           //!< Current PC
        Addr next_pc_ = std::numeric_limits<Addr>::max();           //!< Next PC
        Addr alternate_next_pc_ = std::numeric_limits<Addr>::max(); //!< Alternate next PC if Event was not a change of flow (e.g. branch or trap was not taken)
        ...
        ExcpType excp_type_ = ExcpType::INVALID;                    //!< The exception type for faulting instructions and interrupt Events
        ExcpCode excp_code_ = std::numeric_limits<ExcpCode>::max(); //!< The exception code for faulting instruction and interrupt Events
    ...
    inline std::ostream & operator<<(std::ostream & os, const Event::RegReadAccess & reg_read_access)
    {
    ...
    inline std::ostream & operator<<(std::ostream & os, const Event::RegWriteAccess & reg_write_access)
    {
        ...
        os << " [prev: ";
        for (auto it = reg_write_access.prev_value.rbegin(); it != reg_write_access.prev_value.rend(); ++it)
        {
    ...
    inline std::ostream & operator<<(std::ostream & os, const std::vector<Event::RegReadAccess> & reg_read_accesses)
    {
    ...
    inline std::ostream & operator<<(std::ostream & os, const std::vector<Event::RegWriteAccess> & reg_write_accesses)
    {
    ...
    {
        os << "euid: " << std::dec << event.getEuid()
        << " PC: " << std::setw(16) << std::setfill('0') << std::hex << event.getPc()
        << " " << event.getDisassemblyStr()
