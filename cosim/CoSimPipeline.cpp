...
void CoSimPipeline::setNumHarts(size_t num_harts)
{
    hart_pipelines_.reserve(num_harts);
}

std::unique_ptr<simdb::pipeline::Pipeline> CoSimPipeline::createPipeline(
    simdb::pipeline::AsyncDatabaseAccessor* db_accessor)
{
    ...
    for (HartId hart_id = 0; hart_id < hart_pipelines_.capacity(); ++hart_id)
    {
        ...
        // Note the different std::function signature is due to this being a source function,
        // i.e. <void,T> (different compared to most of the other Functions below)
        ...
        auto async_source = simdb::pipeline::createTask<EventProducerFunction>(
            [pipeline_input_queue, send_evt = Event()](simdb::ConcurrentQueue<Event>& out) mutable -> bool
            {
        ...
        auto convert_to_range = simdb::pipeline::createTask<ConvertToRangeFunction>(
            [](EventBuffer&& evts, simdb::ConcurrentQueue<EventsRange>& out)
            {
                ...
                EventsRange range;
                range.euid_range = std::make_pair(evts.front().getEuid(), evts.back().getEuid());
        ...
        using BoostSerializerFunction = simdb::pipeline::Function<EventsRange, EventsRangeAsBytes>;
        ...
        auto async_writer = db_accessor->createAsyncWriter<CoSimPipeline, EventsRangeAsBytes, EuidRange>(
            [this](EventsRangeAsBytes&& evts,
...
////////////////////////////////////////////////////////////////////////////////////////
void CoSimPipeline::setSnooper(CoSimPipelineSnooper* snooper)
{
    snooper_ = snooper;
}
...
uint64_t CoSimPipeline::CoSimHartPipeline::getLastEventUID() const
{
...
bool CoSimPipeline::simStopped(HartId hart_id) const
{
    return sim_stopped_.at(hart_id);
}
...
std::unique_ptr<Event> CoSimPipeline::CoSimHartPipeline::recreateEventFromDisk(uint64_t euid)
{
    ...
    {
        throw sparta::SpartaException("Internal error occurred. Cannot find event with uid ") << euid << ".";
    }
