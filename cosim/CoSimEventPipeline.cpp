...
void CoSimEventPipeline::createPipeline(simdb::pipeline::PipelineManager* pipeline_mgr)
{
    ...
    auto serialize = simdb::pipeline::createTask<simdb::pipeline::Function<EventList, SerializedEvtsBuffer>>(
        [this](EventList && evts,
               simdb::ConcurrentQueue<SerializedEvtsBuffer> & out,
               bool /*force_flush*/)
        {
    ...
    auto zlib = simdb::pipeline::createTask<simdb::pipeline::Function<SerializedEvtsBuffer, SerializedEvtsBuffer>>(
        [](SerializedEvtsBuffer && evts,
           simdb::ConcurrentQueue<SerializedEvtsBuffer> & out,
           bool /*force_flush*/)
        {
    ...
    auto db_writer = db_accessor->createAsyncWriter<CoSimEventPipeline, SerializedEvtsBuffer, void>(
        [](SerializedEvtsBuffer && serialized,
    ...
    *serialize >> *zlib >> *db_writer;
    ...
    pipeline_flusher_ = std::make_unique<simdb::pipeline::RunnableFlusher>(
        *db_mgr_, serialize, zlib, db_writer);
...
void CoSimEventPipeline::setListener(EventListener* listener)
{
    listener_ = listener;
}
...
void CoSimEventPipeline::commitOldest()
{
    sparta_assert(!uncommitted_evts_buffer_.empty(),
                  "No uncommitted events to commit for core " +
...
void CoSimEventPipeline::commitUpTo(uint64_t euid)
{
    ...
    auto it = std::find_if(
        uncommitted_evts_buffer_.begin(), uncommitted_evts_buffer_.end(),
        [euid](const Event & evt)
        {
            return evt.getEuid() == euid;
        });
...
void CoSimEventPipeline::flush(uint64_t euid, bool flush_younger_only, CoSimObserver* observer, PegasusState* state)
{
    // If the euid is <= last committed, we cannot flush it
    if (last_committed_event_uid_.isValid() &&
        euid <= last_committed_event_uid_.getValue())
    {
        throw simdb::DBException("Cannot flush event with euid " + std::to_string(euid) +
            " for core " + std::to_string(core_id_) + ", hart " + std::to_string(hart_id_) +
...
bool CoSimEventPipeline::simStopped() const
{
    return sim_stopped_;
}
...
void CoSimEventPipeline::preTeardown()
{
    ...
    if (!uncommitted_evts_buffer_.empty())
    {
        std::cout << "At sim end, event pipeline at core " << core_id_
                  << ", hart " << hart_id_
                  << " had " << uncommitted_evts_buffer_.size()
                  << " uncommitted events.\n";
    }
}
...
void CoSimEventPipeline::postTeardown()
{
    ...
    if (avg_us_recreating_evts_from_pipeline_.count())
    {
        ...
        std::cout << "        From serialize task queue:  " << num_pipeline_evts_snooped_in_serialize_task_ << "\n";
        std::cout << "        From zlib task queue:       " << num_pipeline_evts_snooped_in_zlib_task_ << "\n";
        std::cout << "        From DB task queue:         " << num_pipeline_evts_snooped_in_db_task_ << "\n";
    }
    else
    {
