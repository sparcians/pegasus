...
namespace atlas
{
    ...
    {
      public:
        InstSnapshotter(simdb::ObjectManager* obj_mgr, int hart, const std::string & rs1_name,
                        uint64_t rs1_val, const std::string & rs2_name, uint64_t rs2_val,
                        const std::string & rd_name, uint64_t rd_val_before, uint64_t rd_val_after,
                        uint64_t cosim_rd_val_after, int has_imm, uint64_t imm,
                        const std::string & disasm, const std::string & mnemonic, uint64_t opcode,
                        uint64_t pc, int priv, int result_code) :
            obj_mgr_(obj_mgr),
            hart_(hart),
            rs1_name_(rs1_name),
            rs1_val_(rs1_val),
        ...
        void completeTask() override
        {
            ...
            tbl->createObjectWithArgs(
                "HartId", hart_, "Rs1", rs1_name_, "Rs1Val", rs1_val_, "Rs2", rs2_name_, "Rs2Val",
                rs2_val_, "Rd", rd_name_, "RdValBefore", rd_val_before_, "RdValAfter",
                rd_val_after_, "TruthRdValAfter", cosim_rd_val_after_, "HasImm", has_imm_, "Imm",
                imm_, "Disasm", disasm_, "Mnemonic", mnemonic_, "Opcode", opcode_, "PC", pc_,
                "Priv", priv_, "ResultCode", result_code_);
        }
    ...
    {
      public:
        CsrValuesSnapshotter(
            simdb::ObjectManager* obj_mgr, int hart, uint64_t pc,
            const std::vector<std::tuple<std::string, uint64_t, uint64_t>> & all_csr_vals) :
            obj_mgr_(obj_mgr),
            hart_(hart),
            pc_(pc),
        ...
        void completeTask() override
        {
            ...
            for (const auto & tup : all_csr_vals_)
            {
                ...
                tbl->createObjectWithArgs("HartId", hart_, "PC", pc_, "CsrName", csr_name,
                                          "AtlasCsrVal", atlas_csr_val, "CosimCsrVal",
                                          cosim_csr_val);
            }
        }
