...
namespace atlas
{
    ...
    ActionGroup* RvdInsts::fcvt_wu_d_64_handler(atlas::AtlasState* state)
    {
        const AtlasInstPtr & inst = state->getCurrentInst();
        const uint64_t rs1_val = inst->getRs1()->dmiRead<uint64_t>();
        inst->getRd()->write(f64_to_ui32(float64_t{rs1_val}, getRM(inst, state), true));
        update_csr(state);
        return nullptr;
    }
    ...
    ActionGroup* RvdInsts::fcvt_w_d_64_handler(atlas::AtlasState* state)
    {
        ...
        const uint64_t rs1_val = inst->getRs1()->dmiRead<uint64_t>();
        inst->getRd()->write(
            signExtend<int32_t, int64_t>(f64_to_i32(float64_t{rs1_val}, getRM(inst, state), true)));
        update_csr(state);
        return nullptr;
    }
    ...
    ActionGroup* RvdInsts::fcvt_lu_d_64_handler(atlas::AtlasState* state)
    {
        const AtlasInstPtr & inst = state->getCurrentInst();
        const uint64_t rs1_val = inst->getRs1()->dmiRead<uint64_t>();
        inst->getRd()->write(f64_to_ui64(float64_t{rs1_val}, getRM(inst, state), true));
        update_csr(state);
        return nullptr;
    }
    ...
    ActionGroup* RvdInsts::fcvt_l_d_64_handler(atlas::AtlasState* state)
    {
        const AtlasInstPtr & inst = state->getCurrentInst();
        const uint64_t rs1_val = inst->getRs1()->dmiRead<uint64_t>();
        inst->getRd()->write(f64_to_i64(float64_t{rs1_val}, getRM(inst, state), true));
        update_csr(state);
        return nullptr;
    }
